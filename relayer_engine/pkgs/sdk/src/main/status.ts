...
import { getCoreRelayer, getCoreRelayerAddressNative, RPCS_BY_CHAIN, GUARDIAN_RPC_HOSTS } from "../consts"
...
export async function getDeliveryInfoBySourceTx(infoRequest: InfoRequest): Promise<DeliveryInfo[]> {
  const sourceChainProvider = infoRequest.sourceChainProvider || new ethers.providers.StaticJsonRpcProvider(RPCS_BY_CHAIN[CHAIN_ID_TO_NAME[infoRequest.sourceChain]]);
  if(!sourceChainProvider) throw Error("No default RPC for this chain; pass in your own provider (as sourceChainProvider)")
  const receipt = await sourceChainProvider.getTransactionReceipt(infoRequest.sourceTransaction)
  if(!receipt) throw Error("Transaction has not been mined")
  const bridgeAddress = CONTRACTS[infoRequest.environment][CHAIN_ID_TO_NAME[infoRequest.sourceChain]].core
  const coreRelayerAddress = getCoreRelayerAddressNative(infoRequest.sourceChain, infoRequest.environment)
  ...
  //const parsedInstruction = CoreRelayer__factory.connect(coreRelayerAddress, sourceChainProvider).decodeDeliveryInstructionsContainer(deliveryLog.log.data);
  //console.log(parsedInstruction)

  /* Potentially use 'guardianRPCHosts' to get status of VAA; code in comments at end [1] */

  const targetChainProvider = infoRequest.targetChainProvider || new ethers.providers.StaticJsonRpcProvider(RPCS_BY_CHAIN[CHAIN_ID_TO_NAME[infoRequest.targetChain]]);
  if(!targetChainProvider) throw Error("No default RPC for this chain; pass in your own provider (as targetChainProvider)")
...
async function pullEventsBySourceSequence(
  ...
  return await transformDeliveryEvents(
    await coreRelayer.queryFilter(deliveryEvents, 0, 'latest'), targetChainProvider
  )
}
